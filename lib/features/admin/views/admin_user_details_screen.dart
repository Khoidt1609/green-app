...
class AdminUserDetailsScreen extends ConsumerWidget {
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: isActive
                  ? (isAdmin ? Colors.amber.shade50 : Colors.green.shade50)
                  : Colors.red.shade50,
              borderRadius: BorderRadius.circular(20),
              border: Border.all(
                color: isActive
                    ? (isAdmin ? Colors.amber.shade200 : Colors.green.shade200)
                    : Colors.red.shade200,
              ),
            ),
            child: Text(
              isActive
                  ? (isAdmin ? "🛡️ QUẢN TRỊ VIÊN" : "NGƯỜI DÙNG")
                  : "TÀI KHOẢN ĐANG KHÓA",
              style: TextStyle(
                color: isActive
                    ? (isAdmin ? Colors.amber.shade900 : Colors.green.shade800)
                    : Colors.red.shade800,
                fontWeight: FontWeight.bold,
                fontSize: 12,
                letterSpacing: 1,
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            _buildInfoRow(
              "Điểm tuần/tháng",
              "${user.weekPoints} / ${user.monthPoints}",
            ),
          ],
        ),
      ),
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            _buildInfoRow(
              "Ngân hàng",
              user.bankInfo?.bankCode ?? "Chưa liên kết",
            ),
        ...
        child: Column(
          children: [
            ...
            _buildActionButton(
              ...
              onPressed: !isActive
                  ? null
                  : () => _showConfirmDialog(context, ref, "role", isAdmin),
            ),
            const SizedBox(height: 12),
            ...
            _buildActionButton(
              ...
              onPressed: () =>
                  _showConfirmDialog(context, ref, "status", !isActive),
            ),
          ],
        ),
      ),
  ...
  void _showConfirmDialog(
    ...
    if (type == "role") {
      ...
      content =
          "Bạn có chắc chắn muốn thay đổi đặc quyền quản trị của người dùng này?";
    } else {
            ...
            onPressed: () {
              if (type == "role") {
                ref
                    ...
                    .changeRole(user.uid, currentValue ? 'user' : 'admin');
              } else {
                ref
                    .read(adminUserActionProvider.notifier)
                    .toggleStatus(user.uid, !currentValue);
              }
              Navigator.pop(ctx);
    ...
    return submissionsAsync.when(
      data: (list) => list.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (ctx, i) => _buildHistoryTile(
                list[i].taskTitle,
                list[i].createdAt,
                "+${list[i].pointsReward}",
                list[i].status,
  ...
  Widget _buildHistoryTile(
    ...
    Color statusColor = status == 'approved' || status == 'completed'
        ? Colors.green
